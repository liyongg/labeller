...
def next(col="text", init=False):
    ...
    if empty_label_rows.shape[0] == 0:
        st.info("All data are labelled! Consider downloading the CSV using 'Download CSV' in the sidebar. Cycling randomly now.")
...
def main():
    ...
    if button_start:
        ...
        st.session_state.row = next("text", init=True)
