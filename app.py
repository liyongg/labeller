...
def next_item(col="text", init=False):
    if not init:
        st.session_state.df.loc[st.session_state.row_index,
                                "label"] = st.session_state.radio_choice
...
def main():
    with st.sidebar:
        ...
        input_labels = st.slider(
            "How many labels?", min_value=2, max_value=10, step=1)
    ...
    if st.session_state.get('df') is None:
        ...
        st.session_state.radio_choice = st.radio(
            "Label", options=st.session_state.labels)
        ...
        st.write(
            f"**{st.session_state.df.label.isnull().sum()}** records remain to be labelled")

        st.sidebar.download_button(
            "Download CSV", data=download_data(), file_name="output.csv", mime="text/csv")
